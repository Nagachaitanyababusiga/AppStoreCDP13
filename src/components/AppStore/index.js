...
class AppStore extends Component {
  state = {inputContent: '', tabId: 'SOCIAL'}
  ...
  render() {
    const {inputContent, tabId} = this.state
    const filteredList = appsList.filter(
      x =>
        x.appName.toLowerCase().includes(inputContent.toLowerCase()) &&
        x.category === tabId,
    )
    ...
    return (
      <div className="cont-1">
        ...
        <ul className="app-cont">
          {filteredList.map(x => (
            <AppItem key={x.appId} appDetails={x} />
          ))}
        </ul>
      </div>
    )
